 ...
 */
public class JDA4Handler extends CommandHandler {
    ...
    private void handleMessageCreate(final MessageReceivedEvent event) {
        ...
        if (!hasPermission(event.getAuthor(), commandAnnotation.requiredPermissions())) {
            if (Sdcf4jMessage.MISSING_PERMISSIONS.getMessage() != null) {
                event.getChannel().sendMessage(Sdcf4jMessage.MISSING_PERMISSIONS.getMessage()).queue(m -> {
                    m.delete().reason("no permission to use command").queueAfter(5, TimeUnit.SECONDS);
                    event.getMessage().delete().reason("no permission to use command").queueAfter(5, TimeUnit.SECONDS);
                });
            }
            return;
        }
